...
describe('CALL_SUMMARY incremental reuse gate (U-C5)', () => {
  it('INCREMENTAL_SCHEMA_VERSION is bumped to 31 (Swift member-containment DDL)', () => {
  ...
  it('a pre-current stamp fails the `=== INCREMENTAL_SCHEMA_VERSION` reuse gate → forces full re-analyze', () => {
    ...
    // A pre-v31 (v30) index predates the Swift member-containment relation
    // tables, so an incremental top-up emitting one of those edges would fail
    // at the database layer → must NOT reuse.
