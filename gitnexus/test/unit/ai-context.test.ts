...
describe('generateAIContextFiles', () => {
  ...
  it('emits the project-local runner command and drops .gitnexus/run.cjs regardless of mode (#1945)', async () => {
    ...
    try {
      ...
      expect(agentsContent).toContain('`node .gitnexus/run.cjs analyze --index-only`');
  ...
  it('keeps optional comparison guidance in AGENTS.md and imports it from CLAUDE.md', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'gn-optional-review-'));
    try {
      const storage = path.join(dir, '.gitnexus');
      await fs.mkdir(storage, { recursive: true });
      await generateAIContextFiles(dir, storage, 'P', { nodes: 50 }, undefined, {
        defaultBranch: 'develop',
      });
      const agents = await fs.readFile(path.join(dir, 'AGENTS.md'), 'utf-8');
      expect(agents).toContain('**Optional regression review:**');
      expect(agents).toContain('detect_changes({scope: "compare", base_ref: "develop"})');
      expect(agents).toContain('Prefer normal editor tools');
      expect(agents).not.toMatch(/\b(?:MUST|NEVER|ALWAYS)\b/);
      const claude = await fs.readFile(path.join(dir, 'CLAUDE.md'), 'utf-8');
      expect(claude).toContain('@AGENTS.md');
      expect(claude).not.toContain('detect_changes');
    } finally {
      await fs.rm(dir, { recursive: true, force: true });
    }
  });

  it('uses the configured runner for an optional stale-index refresh', () => {
    const content = generateGitNexusContent(
      'TransportFallback',
      { nodes: 50 },
      {
    ...
    expect(content).toContain('node .custom/gitnexus-runner.cjs analyze --index-only');
    expect(content).toContain('**Optional PDG analysis:**');
    expect(content).not.toContain('node .gitnexus/run.cjs');
  });
  ...
  it('keeps graph-result limitations without requiring a graph workflow', () => {
    const content = generateGitNexusContent('RiskScaleProject', {
      nodes: 50,
      edges: 100,
      processes: 5,
    });
    expect(content).toContain('risk: UNKNOWN');
    expect(content).toContain('incomplete evidence');
    expect(content).not.toContain('MUST warn');
  });
...
describe('count-only reindex does not churn the committed block (#2907)', () => {
  ...
  it('leaves both files byte-identical when only the counts moved', async () => {
    ...
    expect(before.claude).not.toContain('(10 symbols, 20 relationships, 3 execution flows)');
  });
  ...
  it('keeps an already count-free block stable under --no-stats', async () => {
    const result = await generateAIContextFiles(
      dir,
      storage,
      'RenamedProject',
    ...
    expect(content).toContain('This repo is indexed as **RenamedProject**.');
...
describe('--no-stats drops the per-cluster symbol counts too (#2907)', () => {
  ...
  it('omits per-cluster counts with either noStats setting', () => {
